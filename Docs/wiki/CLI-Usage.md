...
| `-Rerun` | Interactively re-run a recent job. Shows a numbered list of completed and failed jobs, lets you pick one, and re-runs it with `-Force -Wait`. For multi-server setups sharing a queue, set `paths.downloadRoot` in config to translate download paths between servers. Note: TBA-rejected Sonarr imports are automatically retried after 48 hours; manual -Rerun is only needed if the automatic retry also fails. |
